...
class ConfigManager {
  ...
  private mergeWithEnv(config: AppConfig): AppConfig {
    return {
      ...config,
      database: {
        ...config.database,
      ...
      server: {
        ...
        main: {
          ...config.server.main,
          port: parseInt(
            process.env.PORT || config.server.main.port.toString()
          ),
          url: process.env.NEXTAUTH_URL || config.server.main.url,
        },
        agent: {
          ...config.server.agent,
          port: parseInt(
            process.env.AGENT_PORT || config.server.agent.port.toString()
          ),
        },
      },
      llm: {
